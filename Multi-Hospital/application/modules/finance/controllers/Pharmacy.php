...
class Pharmacy extends MX_Controller
{
    ...
    public function getMedicinesWithBatches()
    {
        ...
        $this->db->select('m.id, m.name, m.generic, m.company, m.s_price, 
                          mb.batch_number, mb.expiry_date, mb.current_stock, 
                          mb.selling_price as batch_selling_price,
        ...
        $this->db->where('mb.expiry_date >', date('Y-m-d')); // Only non-expired
        
        if (!empty($searchTerm)) {
            $this->db->group_start();
            $this->db->like('m.name', $searchTerm);
            $this->db->or_like('m.generic', $searchTerm);
            $this->db->or_like('mb.batch_number', $searchTerm);
            $this->db->group_end();
        }
        
        $this->db->order_by('m.name', 'asc');
        $this->db->order_by('mb.expiry_date', 'asc'); // Show expiring first
        $this->db->limit(50);
        ...
        foreach ($medicines as $medicine) {
            ...
            if ($days_to_expiry <= 30) {
                $expiry_status = ' (Expires in ' . round($days_to_expiry) . ' days)';
            }
            
            $batch_info = $medicine->batch_number ? ' [Batch: ' . $medicine->batch_number . ']' : '';
            $stock_info = ' (Stock: ' . $medicine->current_stock . ')';
            $price = $medicine->batch_selling_price ?: $medicine->s_price;
            
            $response[] = array(
                'id' => $medicine->id . '*' . $price . '*' . $medicine->name . '*' . $medicine->company . '*' . $medicine->current_stock . '*' . $medicine->batch_number,
                'text' => $medicine->name . $batch_info . $stock_info . $expiry_status,
