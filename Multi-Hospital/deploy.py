...
def collect_files(base_dir, only_changed=False, skip_assets=False):
    files = []
...
def deploy(dry_run=False, only_changed=False, skip_assets=False, resume=False):
    ...
    print(f"[*] Mode: {'changed only' if only_changed else 'full'}"
          f"{' | skip-assets' if skip_assets else ''}"
          f"{' | resume' if resume else ''}")
    print()

    files = collect_files(base_dir, only_changed=only_changed, skip_assets=skip_assets)
...
if __name__ == '__main__':
    ...
    deploy(dry_run=args.dry_run, only_changed=args.only_changed,
           skip_assets=args.skip_assets, resume=args.resume)
