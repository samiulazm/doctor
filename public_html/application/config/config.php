...
$ci_base_url_strict = getenv('CI_BASE_URL_STRICT');
if (
	($ci_base_url_strict === false || $ci_base_url_strict === '')
	&& $ci_base_url !== false && $ci_base_url !== ''
	&& function_exists('ci_stale_ci_base_url_should_use_request')
	&& ci_stale_ci_base_url_should_use_request($ci_base_url)
) {
	// e.g. .env still has old http://IP/~user/ while the site is served on a real domain
	$ci_base_url = '';
}
if ($ci_base_url !== false && $ci_base_url !== '') {
	$config['base_url'] = preg_match('#/$#', $ci_base_url) ? $ci_base_url : $ci_base_url . '/';
} else {
	$config['base_url'] = ci_base_url_from_request();
}
