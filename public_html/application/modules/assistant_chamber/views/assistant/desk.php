...
<div class="content-wrapper">
    <section class="content-header"><h1>Check-in desk &amp; queue</h1></section>
    <section class="content">
        ...
        <form method="get" class="form-inline mb-3">
            ...
            <a class="btn btn-default ml-2" href="<?php echo site_url('assistant_chamber/manual_booking'); ?>">Manual booking</a>
            <?php if ($doctor_id && $chamber_id) : ?>
            <a class="btn btn-default ml-2" href="<?php echo site_url('assistant_chamber/bulk_sms?from_desk=1&doctor_id=' . (int) $doctor_id . '&chamber_id=' . (int) $chamber_id . '&date=' . rawurlencode($queue_date)); ?>">Bulk SMS (queue phones)</a>
            <?php endif; ?>
        </form>
        <?php if (!empty($queue)) : ?>
                ...
                <tr data-id="<?php echo (int) $q->id; ?>">
                    ...
                    <td>
                        ...
                        <form method="post" action="<?php echo site_url('assistant_chamber/checkin'); ?>" style="display:inline">
                            ...
                            <button class="btn btn-xs btn-success" type="submit">Arrived</button>
                        </form>
                        <?php endif; ?>
                        <form method="post" action="<?php echo site_url('assistant_chamber/mark_serving'); ?>" style="display:inline">
                            <input type="hidden" name="<?php echo $this->security->get_csrf_token_name(); ?>" value="<?php echo $this->security->get_csrf_hash(); ?>">
                            <input type="hidden" name="queue_id" value="<?php echo (int) $q->id; ?>">
                            <button class="btn btn-xs btn-primary" type="submit">Serving</button>
                        </form>
                        <form method="post" action="<?php echo site_url('assistant_chamber/emergency_bump'); ?>" style="display:inline">
                            <input type="hidden" name="<?php echo $this->security->get_csrf_token_name(); ?>" value="<?php echo $this->security->get_csrf_hash(); ?>">
                            <input type="hidden" name="queue_id" value="<?php echo (int) $q->id; ?>">
                            <button class="btn btn-xs btn-warning" type="submit">Emergency bump</button>
                        </form>
                        <button class="btn btn-xs btn-default" type="button" data-toggle="collapse" data-target="#v<?php echo (int) $q->id; ?>">Vitals</button>
                        <?php if (!empty($q->patient_id)) : ?>
                        <a class="btn btn-xs btn-info" target="_blank" title="Latest saved Rx for this patient" href="<?php echo site_url('assistant_chamber/print_latest_rx?patient_id=' . (int) $q->patient_id . '&doctor_id=' . (int) $q->doctor_id . '&chamber_id=' . (int) $chamber_id . '&date=' . rawurlencode($queue_date)); ?>">Print latest Rx</a>
                        <?php endif; ?>
        ...
        <button type="button" class="btn btn-outline-secondary" id="saveOrder">Save queue order</button>
        ...
        <?php else : ?>
            <?php if ($doctor_id && $chamber_id) : ?>
                <p class="text-muted">No queue entries.</p>
            <?php endif; ?>
        <?php endif; ?>
    </section>
</div>
