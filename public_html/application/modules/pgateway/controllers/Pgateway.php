...
class pgateway extends MX_Controller {
    ...
    public function addNewSettings() {
        ...
        if ($pgateway->name == 'SSLCommerz') {
            $this->form_validation->set_rules('merchant_key', 'Store ID', 'trim|required|min_length[1]|max_length[200]|xss_clean');
            $this->form_validation->set_rules('salt', 'Store Password', 'trim|required|min_length[1]|max_length[200]|xss_clean');
        }
        if ($pgateway->name == 'bKash') {
            $this->form_validation->set_rules('public_key', 'App Key', 'trim|required|min_length[1]|max_length[200]|xss_clean');
            $this->form_validation->set_rules('secret', 'App Secret', 'trim|required|min_length[1]|max_length[500]|xss_clean');
            $this->form_validation->set_rules('APIUsername', 'bKash Username', 'trim|required|min_length[1]|max_length[100]|xss_clean');
            $this->form_validation->set_rules('APIPassword', 'bKash Password', 'trim|required|min_length[1]|max_length[100]|xss_clean');
        }
