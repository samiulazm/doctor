...
class Cronjobs extends MX_Controller
{
    ...
    /**
     * Chamber practice: send SMS for medicine reminders whose dose time matches current hour.
     * Cron: curl -s "https://yoursite/index.php/cronjobs/chamberMedicineReminders"
     */
    public function chamberMedicineReminders()
    {
        ...
        foreach ($rows as $r) {
            if (!chamber_practice_enabled_for_hospital($this, (int) $r->hospital_id)) {
                continue;
            }
            $times = json_decode($r->dose_times_json, true);
            if (!is_array($times)) {
                continue;
            }
            $match = false;
            foreach ($times as $t) {
                if (strpos($t, ':') !== false && substr($t, 0, 5) === substr($hour, 0, 5)) {
                    $match = true;
                    break;
                }
            }
            ...
            $this->db->where('id', $r->patient_id);
            $this->db->where('hospital_id', $r->hospital_id);
            $pat = $this->db->get('patient')->row();
            if (!$pat || empty($pat->phone)) {
                continue;
            }
            $msg = 'Reminder: take ' . $r->medicine_label . ' now.';
            if ($this->chamber_platform_model->sendSmsMessage($r->hospital_id, $pat->phone, $msg)) {
                $this->db->where('id', $r->id);
                $this->db->update('medicine_reminder', array('last_sent_at' => date('Y-m-d H:i:s')));
            }
        }
        echo 'ok';
    }
    ...
    /**
     * Log hospitals whose chamber SMS ledger balance is below threshold (default 20).
     * Cron: curl -s "https://yoursite/index.php/cronjobs/chamberSmsLowBalance?threshold=20"
     */
    public function chamberSmsLowBalance()
    {
        $threshold = (int) $this->input->get('threshold');
        if ($threshold < 1) {
            $threshold = 20;
        }
        $this->load->model('portal/chamber_platform_model');
        $rows = $this->db->query('SELECT DISTINCT hospital_id FROM sms_credit_ledger')->result();
        foreach ($rows as $row) {
            $hid = (int) $row->hospital_id;
            $bal = $this->chamber_platform_model->getSmsBalance($hid, null);
            if ($bal < $threshold) {
                log_message('error', 'Chamber SMS low balance: hospital_id=' . $hid . ' balance=' . $bal . ' threshold=' . $threshold);
            }
        }
        echo 'ok';
    }
