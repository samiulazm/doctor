    ...
    function getSymptomList()
    {
        ...
        foreach ($data['symptoms'] as $symptom) {
            ...
            if ($this->ion_auth->in_group(array('admin'))) {
                ...
                $options1 = '<a type="button" class="btn btn-primary btn-sm editbutton" title="' . lang('edit') . '" data-bs-toggle="modal" data-id="' . $symptom->id . '"><i class="fa fa-edit"></i> ' . lang('edit') . '</a>';
            }

            if ($this->ion_auth->in_group(array('admin'))) {
                // The 'btn btn-danger' classes make the button red, indicating a potentially dangerous action.
                $options5 = '<a class="btn btn-danger btn-sm delete_button" title="' . lang('delete') . '" href="symptom/delete?id=' . $symptom->id . '" onclick="return confirm(\'Are you sure you want to delete this item?\');"><i class="fa fa-trash"></i> ' . lang('delete') . '</a>';
            }
